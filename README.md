<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!--          ⚡ UMAIR HABIB · アニメ × コード · NEON DEV TERMINAL ⚡          -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

<!-- ░░░ HERO BANNER ░░░ -->
<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:7C3AED,50:EC4899,100:06B6D4&height=240&section=header&text=Umair%20Habib&fontSize=72&fontColor=FFFFFF&animation=fadeIn&fontAlignY=38&desc=%E2%9C%A6%20%20Frontend%20Dev%20%C2%B7%20Anime%20Soul%20%C2%B7%20Plus%20Ultra%20%20%E2%9C%A6&descSize=18&descAlignY=58" alt="banner" />
</p>


<!-- ░░░ TYPING ░░░ -->
<p align="center">
  <a href="https://github.com/Umair-Habibx123">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&duration=2600&pause=700&color=EC4899&center=true&vCenter=true&width=760&height=80&lines=%24+sudo+npm+run+plus-ultra+%F0%9F%94%A5;%E3%82%88%E3%81%86%E3%81%93%E3%81%9D+%2F%2F+Welcome+to+the+Hidden+Dev+Village+%E2%9C%A8;Compiling+dreams+into+pixel-perfect+UIs...;Code+like+a+shonen+protagonist+%E2%80%94+never+give+up." alt="Typing SVG" />
  </a>
</p>

<!-- ░░░ STATUS BADGES ░░░ -->
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Umair-Habibx123&style=for-the-badge&color=EC4899&label=POWER+LEVEL" alt="views" />
  <img src="https://img.shields.io/github/followers/Umair-Habibx123?style=for-the-badge&color=7C3AED&label=ALLIES&logo=github&logoColor=white" alt="followers" />
  <img src="https://img.shields.io/badge/Bankai-ACTIVATED-06B6D4?style=for-the-badge&logo=lightning&logoColor=white" alt="status" />
</p>

<div align="center">

`▸ booting dev_terminal.exe ────────────────────── [ ████████████ 100% ]`

</div>

<p align="center">
  <img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</p>

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## ⛩️ `~/whoami --verbose`

```typescript
const umair: Developer = {
  name:     "Umair Habib",
  alias:    "コードの勇者",              // Hero of Code
  class:    "Frontend Mage 🔮",
  location: "Pakistan 🇵🇰",
  stack:    ["React", "Next.js", "TypeScript", "Tailwind"],
  backend:  ["Node.js", "Express", "MongoDB", "Firebase"],
  fuel:     ["☕ coffee", "🍜 ramen", "📺 anime", "🎵 lo-fi"],
  openTo:   ["Freelance", "Collabs", "Open Source"],
  motto:    () => "If it's not shipped, it doesn't exist 🚀",
};
```

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## ⚔️ `import { Arsenal } from './skills'`

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,tailwind,html,css,redux&theme=dark&perline=8" />
  <br/>
  <img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,firebase,git,github,vite,figma&theme=dark&perline=8" />
</p>

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 📊 `git log --stat --battle-records`

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=Umair-Habibx123&show_icons=true&theme=synthwave&include_all_commits=true&count_private=true&hide_border=true&title_color=EC4899&icon_color=06B6D4&text_color=ffffff&bg_color=0D1117" />
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Umair-Habibx123&layout=compact&theme=synthwave&hide_border=true&title_color=EC4899&text_color=ffffff&bg_color=0D1117" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Umair-Habibx123&theme=synthwave&hide_border=true&background=0D1117&stroke=EC4899&ring=06B6D4&fire=7C3AED&currStreakLabel=EC4899" alt="streak" />
</p>

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🏆 `./unlock-achievements.sh`

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Umair-Habibx123&theme=algolia&no-frame=true&no-bg=true&column=7&margin-w=6&margin-h=6" alt="trophies" />
</p>

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🐉 `render contribution-graph --live`

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Umair-Habibx123&bg_color=0D1117&color=EC4899&line=7C3AED&point=06B6D4&area=true&area_color=7C3AED&hide_border=true&custom_title=Umair's%20Contribution%20Arc" alt="activity graph" width="100%" />
</p>

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🌙 `cat lore.txt`

```bash
~/umair $ cat lore.txt

[ FACTS ]
  🌙  The best code is written after midnight
  🐛  20% coding · 80% figuring out why it worked
  🔥  "Plus Ultra" — I push past my limits, every commit
  🎌  Powered by anime openings on loop
  🚀  Ship it, then perfect it
```

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 💬 `echo $QUOTE`

<p align="center">
  <em><strong>「 Hard work betrays none, but dreams betray many. 」</strong></em><br/>
  <sub>努力は人を裏切らない · Code on, Senpai. ⚔️</sub>
</p>

<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 📡 `connect --to umair`

<p align="center">
  <a href="mailto:umairhabibabc@gmail.com">
    <img src="https://img.shields.io/badge/Email-EC4899?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://github.com/Umair-Habibx123">
    <img src="https://img.shields.io/badge/GitHub-7C3AED?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="https://www.linkedin.com/in/umair-habib-developer/">
    <img src="https://img.shields.io/badge/LinkedIn-06B6D4?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://umairhabib.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-Visit-7C3AED?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
  </a>
</p>

<p align="center">
  <img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</p>

<!-- ░░░ FOOTER ░░░ -->
<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:06B6D4,55:EC4899,100:7C3AED&height=130&section=footer&text=Thanks%20for%20visiting%20%E2%80%94%20now%20go%20write%20legendary%20code%20%E2%9A%A1&fontSize=15&fontColor=ffffff&fontAlignY=78" />
</p>
